...
public class StudentRepositoryImpl implements StudentRepositoryCustom {
    ...
    private static final String SQL_GET_ALL_CUSTOM = "select s.*,sp.`name` as study_program_name, u.`name` as university_name from student s inner join study_program sp on s.study_program_id=sp.id inner join university u on s.university_id=u.id where s.active=1";

    private static final String SQL_GET_BY_ID_CUSTOM = "select s.*,sp.`name` as study_program_name, u.`name` as university_name from student s inner join study_program sp on s.study_program_id=sp.id inner join university u on s.university_id=u.id where s.active and s.id=?";
