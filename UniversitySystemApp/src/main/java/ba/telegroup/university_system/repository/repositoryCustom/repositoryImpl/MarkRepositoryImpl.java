...
public class MarkRepositoryImpl implements MarkRepositoryCustom {
    ...
    private static final String SQL_GET_ALL_CUSTOM = "select m.*, s.first_name as student_name, ss.`name` as school_subject_name, t.first_name as teacher_name from mark m inner join student s on m.student_id = s.id  inner join school_subject ss on m.school_subject_id = ss.id inner join teacher t on m.teacher_id = s.id where s.active=1 and ss.active=1 and t.active=1";
    private static final String SQL_GET_BY_ID_CUSTOM = "select m.*, s.first_name as student_name, ss.`name` as school_subject_name, t.first_name as teacher_name from mark m inner join student s on m.student_id = s.id  inner join school_subject ss on m.school_subject_id = ss.id inner join teacher t on m.teacher_id = s.id where s.active=1 and ss.active=1 and t.active=1 and m.id=?";
